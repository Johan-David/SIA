...
public:
    ...
    void preprocess(const Scene *scene)
    {
        ...
        while(m_photonMap->size() < m_photonCount){
            ...
            float albedo = -1.0;
            float pdf;
            while ((albedo == -1.0 || albedo > Eigen::internal::random<float>(0, 1))
                                      && m_photonMap->size() < m_photonCount && p.getPower().mean() > Epsilon){
                ...
                if((material->reflectivity() > 1e-6).any()) {
                    ...
                    color *= material->transmissivness() * cos_term;
                }

                // refraction
                else if((material->transmissivness() > 1e-6).any()) {
                    ...
                    color *= material->premultBrdf(-ray.direction, r, normal, hit.texcoord());
                }
                albedo = color.mean()/p.getPower().mean();
                if(material->specularSamplingWeight() == 0){
                    m_photonMap->push_back(Photon(ray.at(hit.t()), -ray.direction, p.getPower()));
                }
                p = Photon(ray.at(hit.t()), r, color * albedo);
            }
        }
    ...
    Color3f Li(const Scene *scene, const Ray & ray) const
    {
        ...
        if (hit.foundIntersection())
        {
            ...
            if(material->specularSamplingWeight() == 0) {
                  ...
                  m_photonMap->search(pos, m_photonRadius, vecs);
                  for (std::uint32_t i : vecs) {
                      const Photon &p = (*m_photonMap)[i];
                      const Vector3f x = (pos - p.getPosition()) / m_photonRadius;
                      if(x.norm() <= 1) {
                          const Color3f brdf = material->premultBrdf(-ray.direction, p.getDirection(), normal, hit.texcoord());
                          float simpson = 3.0/M_PI * ((1.0 - x.norm() * x.norm()) * (1.0 - x.norm() * x.norm()));
                          radiance += simpson * p.getPower() * brdf;
                      }
                  }
                  radiance *= (1.0 / (m_nPaths * m_photonRadius * m_photonRadius));
                  return radiance;
                }
